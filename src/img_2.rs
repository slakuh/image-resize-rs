...
impl ImageSize 
{
    ...
    fn resize_to_width(&mut self, new_width: u32) -> bool//,  rsz_type: &Resize)
    {
    ...
    fn resize_to_height(&mut self, new_height: u32) -> bool//, rsz_type: &Resize)
    {
        let calc_new_size;// = false;
    ...
    //TODO: ako ne doće do promjene veličeine i file je isti samo promijeniti naziv datotoeke
    pub fn resize_image(&mut self)
    {
        ...
        {        
            ...
            else
            {
                ...
                //panic!("Should not happened in img_2::ImageSize::resize_image");
            }
            ...
            if !is_size_changed && image_format_to_string(self.image_format) == self.file_extension()
            {  
            ...
            else
            {
                ...
                println!("Image {:?}\nresized to: {}x{}\n", &self.path.file_name().unwrap(), self.width, self.height);
            }
...
fn out_file_name(path: &Path, image_format: ImageFormat) -> PathBuf
{
    ...
    let out_name = String::from(name) + constants::FILE_NAME_SUFFIX + "." + &image_format_to_string(image_format);
